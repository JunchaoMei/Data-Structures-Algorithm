class Dijkstra_1D_Array
{	//paths starting from vertex-k
	public static void dijkstra_1D_Array (int[] w, Edge[] F, int k)
	{
		int n1 = w.length;
		int n = (int)(Math.sqrt(8*n1+1)+1)/2;
		int[][] W = new int[n+1][n+1];

		//initialize W using 1D-Array
		for (int x=1; x<=n; x++)
		{
			for (int y=1; y<=n; y++)
			{
				W[x][y] = OneD_Array.getWeight_1D(w,x,y);
			}//end for loop y
		}//end for loop x
	...
	public static void main (String[] args) throws Exception
	{
		...
		int[] w = OneD_Array.array_2to1(W);
		dijkstra_1D_Array(w, F, 1);
		printF(F);
		printEdge(F[1]);
